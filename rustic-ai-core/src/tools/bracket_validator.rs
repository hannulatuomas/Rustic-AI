...
#[cfg(test)]
mod tests {
    use super::BracketValidatorTool;
    use crate::config::schema::{PermissionMode, ToolConfig, WorkingDirMode};

    fn tool() -> BracketValidatorTool {
        BracketValidatorTool::new(ToolConfig {
            name: "bracket_validator".to_string(),
            enabled: true,
            ...
            stream_output: true,
            require_sudo: false,
            privileged_command_patterns: vec![],
            read_only_blocked_patterns: vec![],
            taxonomy_membership: vec![],
        })
    }
    ...
    fn validates_balanced_brackets() {
        ...
        let payload = serde_json::json!({
            "content": "fn main() { let x = [1, 2, (3)]; }",
            "language": "rust",
        });

        let result = tool
            .run_operation(&payload)
    ...
    #[test]
    fn reports_mismatched_brackets() {
        let tool = tool();
        let payload = serde_json::json!({
            "content": "fn main(] {",
            "language": "rust",
