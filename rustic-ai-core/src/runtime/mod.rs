...
impl Runtime {
    pub fn new(
        ...
        let tools = Arc::new(ToolManager::new(ToolManagerInit {
            ...
            dynamic_routing_enabled: config.features.dynamic_routing_enabled,
            dynamic_routing: Arc::new(config.dynamic_routing.clone()),
            todo_tracking_enabled: config.features.todo_tracking_enabled,
            project_id: config.project.as_ref().map(|project| project.name.clone()),
